...
test('split right from no split', () => {
  ...
  expect(verticalLayout).toStrictEqual(_verticalLayout())
  expect(splitWindowRight(verticalLayout, 0)).toStrictEqual(nearest3)
  expect(verticalLayout).toStrictEqual(_verticalLayout())
  var a = splitWindowRight(hvlayout, 1)
  var b = splitRight(0, buffer(0), splitBelow(1, splitRight(2, buffer(1), buffer(2)), buffer(3)))
  console.log('a, b', [a, b])
  expect(a).toStrictEqual(b)
  expect(splitWindowBelow(splitWindowRight(hvlayout, 1), 2)).toStrictEqual(nearest1)
})
